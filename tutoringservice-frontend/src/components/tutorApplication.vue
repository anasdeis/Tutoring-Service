...
export default {
  ...
  methods: {
    ...
    approveRow(rowData) {
      AXIOS.patch(
        `http://localhost:8080/tutorApplication/update/${rowData.tutorApplicationID}?isAccepted=true`
      )
        .then(response => {
    ...
    declineRow(rowData) {
      AXIOS.patch(
        `http://localhost:8080/tutorApplication/update/${rowData.tutorApplicationID}?isAccepted=false`
      )
        .then(response => {
          this.errorTutorApplication = "";
        })
    ...
    onFilterSet(filterText) {
      ...
      let data = this.tutorApplications.filter(tutorApplication => {
        return tutorApplication.subject
          .toLowerCase()
